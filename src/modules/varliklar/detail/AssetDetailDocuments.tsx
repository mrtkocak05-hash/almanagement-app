...
import type { AssetDetail, AssetDocument } from '@/types/assets'

interface Props { asset: AssetDetail; onRefresh: () => void }

const DOC_TYPES = ['Fatura', 'Sözleşme', 'Ekspertiz', 'Sigorta', 'Ruhsat', 'Diğer']

export function AssetDetailDocuments({ asset, onRefresh }: Props) {
  ...
  const docs = asset.documents.filter(d => !d.deleted_at)
  ...
  const handleUpload = async () => {
    ...
    try {
      setUploading(true)
      await assetsApi.uploadDocument(asset.id, selectedFile, docType, docTitle.trim())
    ...
    <div className="space-y-4">
      ...
      {showForm && selectedFile && (
        <div className="p-4 rounded-lg border border-border bg-muted/20 space-y-3">
          <p className="text-xs font-semibold text-muted-foreground uppercase">Belge Bilgileri</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input value={docTitle} onChange={e => setDocTitle(e.target.value)} placeholder="Belge adı" />
            </div>
          </div>
      ...
      {docs.length === 0 ? (
        <p className="text-sm text-muted-foreground py-8 text-center">Henüz belge yok.</p>
      ) : (
